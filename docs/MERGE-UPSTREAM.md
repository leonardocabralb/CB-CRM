...
| `src/lib/api/v1/contacts.ts` (`resolveAuditUserId`, decisão 7 da Fase 11, 25/09/2026) | o autor das escritas da API v1 — e o `user_id` das fichas e conversas que `POST /v1/contacts` e `POST /v1/messages` criam (`resolve-conversation.ts`) — é o DONO DA CONTA, e só ele: o original lê `whatsapp_config.user_id` (quem conectou o número) primeiro, e o login dele CASCADEia as linhas. Leitura que falha ou conta sem dono = 500. Um merge que traga a função crua devolve o autor não durável sem conflito. Pino: `dono-duravel.test.ts` ("a API resolve o dono pela CONTA") |
...
| `src/app/api/whatsapp/webhook/route.ts` (7ª linha nossa, decisão 7 da Fase 11, 25/09/2026) | o dono das fichas e conversas que a entrada da Meta cria é o DONO DA CONTA (`donoDaConta`, `src/lib/cb-channels/resolve-inbound.ts`, NOSSO), nunca `config.user_id` (quem conectou o número): `contacts.user_id` e `conversations.user_id` CASCADEiam de `auth.users`, e apagar o login de quem saiu levaria os clientes junto. Sem dono resolvível, as mensagens da entrega são descartadas com log — nunca a queda para quem conectou. O original grava `ownerUserId: config.user_id`, e um merge que traga o bloco cru o devolve sem conflito. Pino: `src/lib/contacts/dono-duravel.test.ts` ("a ingestão resolve o dono pela CONTA") e o teste "conta sem dono resolvível" em `route.bsuid.test.ts` |
...
| `messages.content_type` (o CHECK é do upstream, 0001; estendido pela 0010, pela 0906 e pela 1044) | `'call'` na lista (ligação de WhatsApp, 1044). Um merge que recrie o CHECK tira o valor: a ligação passa a ser recusada com 23514 e some, sem aviso na tela (a Evolution já recebeu 200). A conferência da 1044 exige UM CHECK sobre `content_type` — ver `.claude/rules/ligacoes.md` |
...
| `src/app/(dashboard)/dashboard-shell.tsx` (celular dos membros, 1046) | `useMeuCelular(user?.id)` no topo (a leitura sai junto com a do perfil), `esperandoCelular` no spinner (sem olhar a conta) e, abaixo do `if (!user) return null`, o envoltório `<ExigenciaDoCelular key={user.id}>` em volta da `<PortaDeEntrada>` — decide uma vez, na montagem. Um merge que traga a casca crua tira a exigência sem conflito nenhum |
