...
| `src/lib/presence.ts` (Fase 10c) | `formatLastSeen` recebe o IDIOMA (`LOCALE_DAS_DATAS.code`, nunca o do navegador) e usa `Intl.RelativeTimeFormat`, devolvendo `null` sem data; `presenceLabel` SAIU — a moldura em inglês ("Offline — last seen …") virou `Presence.*`, no hook NOSSO `src/hooks/use-rotulo-de-presenca.ts`. Um merge que traga o arquivo cru devolve o inglês e quebra o hook no typecheck |
