...
- **Merge:** 25/09/2026 às 22:50 UTC (`b70b76d3`), com o `main` conferido (`2f969c65`) e CI verde no `11ae9f30`. Rollout `converged` às 22:56 UTC; `/login` 200 e o cron das agendadas 401 (o segredo no lugar).
...
- **Verificação:** `tsc` limpo; lint nos 60 avisos da base; os testes dos resolvedores reescritos (quem conectou aparece nos dados, sempre diferente do dono); o pino `dono-duravel.test.ts` amarra a origem (nada de `created_by`/`user_id` no resolvedor, todo `ownerUserId` nascido de `donoDaConta`); mutantes 7/7.
