...
- **Fica para depois:** ~~o dono durável da entrada da Meta e da Evolution (PR próprio, decisão 7)~~ — feito no PR `fix/dono-duravel-na-entrada` (abaixo); tratar a troca de identidade que a mensagem de sistema anuncia (`user_changed_user_id`, `user_id_update`); a busca de /contatos e o `?search=` da v1 pelo @; o avatar da ficha sem nome mostra "@" como inicial (o mesmo da ficha só-Instagram).

**Decisão 7 — o dono durável da entrada (PR `fix/dono-duravel-na-entrada`, 25/09/2026):**
