...
>
> **Este documento envelhece.** As linhas citadas foram conferidas contra o `main` de 24/09/2026 (`7a082fdb`, merge do PR #288). Antes de agir numa linha citada, confira de novo.
>
> - **Criado:** 24/09/2026. **Revisado (v2):** 24/09/2026, depois de duas revisões independentes (fatos contra o código; coerência com as decisões e as regras). 28 achados (6 graves, 15 médios, 7 menores), todos incorporados: o que mudou está na seção 10.
...
> - **Fluxo por fase:** branch a partir de `main` → PR para o CB-CRM → revisão em duas lentes → merge. Um PR por fase. Migration aplicada ANTES do merge, depois do replay verde do CI, com número acima do maior no `main` (hoje 1040).

## Estado

| Fase | Escopo | Estado | Migration | PR |
| --- | --- | --- | --- | --- |
| F0 | Medições baratas (links do Asaas, testes que fixam a semântica de hoje) | a fazer | — | — |
...
| F2 | Quem responde: agente de entrada da conexão, agente ativo da conversa, passo "Atribuir agente", pausa por gente, fila de turnos, resposta em texto. **Porta: P1, P2, P6 e P7 respondidas** | a fazer | sim | — |
| F3 | O que cada agente vê (ficha, campos, negócio, cobranças, transcrições) + base de conhecimento por agente | a fazer | sim | — |
...
1. Hoje existe **um** agente de IA para a plataforma. O pedido é uma aba onde se **criam e mantêm vários agentes**, cada um com:
   - **personalidade e instruções** próprias (o prompt);
...
- **`ai_usage_log`: 1.685 linhas, só dos modos `radar` (1.452) e `transcricao` (233).** O assistente de conversa **nunca respondeu ninguém**, e o rascunho e o Playground nunca registraram uso.
- **Base de conhecimento vazia** (`ai_knowledge_documents` e `ai_knowledge_chunks`: 0).
...
- **Upstream:** o original não mexe na IA desde 13/09 (#527) e não tem nada de vários agentes nem ferramentas. Risco de conflito: só com as nossas Fases 10 e 11 do plano do merge (i18n de `ai-usage.tsx`; BSUID em `flows/meta-send.ts`).

### 3.3 O que as integrações já oferecem às ferramentas

- **Asaas:** o espelho guarda `link_fatura` e `link_boleto` por cobrança (`supabase/migrations/0994_cb_asaas_espelho.sql:155-156`; `src/lib/asaas/leitura.ts:132-133`), e a leitura por contato já existe (`src/lib/asaas/espelho.ts`). A 2ª via é **leitura**, sem escrever nada no Asaas. ⚠️ O espelho pode estar velho: a régua **relê cada parcela no Asaas** antes de cobrar, justamente para não cobrar quem pagou (`.claude/rules/integracoes-asaas.md`).
...
| **Configuração dos módulos** | Provedor e modelo do Radar | a linha padrão de `ai_configs`, que perde o papel de "agente" |
| **Agente** | Nome, descrição, instruções, provedor + modelo, conexões, horário, teto, acesso a dados, ferramentas, para quem pode passar | tabela nova |
| **Agente de entrada da conexão** | Quem atende uma conversa que ainda não tem agente, naquela conexão (em geral a triagem), ou "sem IA" (D3). A quem ele atende é a pergunta P1 | coluna nova em `cb_channels` |
...
- **`cb_ia_turnos`** (F2): `conversation_id`, `mensagem_gatilho_id`, `canal_id`, `agentes` (a lista dos que agiram — a passagem fica DENTRO do mesmo turno), `modo` (`producao`/`teste`), `status` (`aguardando`, `rodando`, `respondeu`, `transferiu`, `sem_resposta`, `fora_do_horario`, `pausado_no_meio`, `falhou`), `executar_apos`, `rodando_desde` (a posse), `iteracoes`, `acoes` (jsonb: ferramenta, argumentos, resultado resumido, ok/erro), tokens, `erro`. **Índice único parcial por conversa com `status = 'aguardando'`** (a rajada: um turno esperando por conversa). Fechada ao navegador; a tela lê por rota de admin.
- **`ai_usage_log`** (F1b): `ia_agente_id` (SET NULL) + `ia_agente_nome` (congelado, a regra dos rótulos da 912), `turno_id`; o `CHECK` de `mode` ganha `agente` e `agente_teste`. **Migration antes do código**, senão o insert falha e `logAiUsage` engole.
...
2. **Alguma automação de "nova mensagem" ou "palavra-chave" RODOU para esta mensagem** → **ninguém**, para o cliente não receber duas respostas. Hoje o portão é a mera EXISTÊNCIA dessa automação no canal, mesmo sem ela casar, o que calaria o agente para sempre numa conexão com qualquer palavra-chave. `dispararAutomacoes` já devolve o que rodou.
3. A conversa está pausada → **ninguém**.
4. Há **agente ativo** na conversa, ligado, e a conexão da mensagem está nas `conexoes` dele → **ele**. O responsável humano NÃO cala o agente ativo (a atribuição deixa de ser portão; ver a P2).
5. Senão, a conexão tem **agente de entrada** e a conversa se qualifica para ele (pergunta P1) → **ele**, e ele vira o agente ativo.
6. Senão → **sem IA**.
...
**Passagem entre agentes (D2, D3):** a ferramenta `passar_para_agente` (F4a) aceita só os agentes de `pode_passar_para` que têm a conexão da conversa, lista a descrição de cada um para o modelo escolher, troca o agente ativo e **roda o agente novo no mesmo turno**, com teto de 1 passagem por turno (sem pingue-pongue). A passagem aparece no fio como uma linha de sistema ("IA · Triagem passou para IA · Cobrança"), para cumprir o "registrada na conversa" da D2, e não só na tabela fechada dos turnos. Até a F4a existir, a passagem é só por automação.

**Automação aciona o agente (D9):** um passo NOVO, **"Atribuir agente de IA: X"** — não a mudança do `set_ai` de hoje, que ao ligar apaga o responsável humano. O passo grava o agente ativo e zera o teto, **não toca `assigned_agent_id`**, e trata a pausa pela pergunta P2. O `set_ai` legado continua como está (ligar/desligar), e o "Retomar IA" do cabeçalho deixa de soltar a atribuição, porque ela deixou de ser portão. A régua do Asaas poder usar o passo é a pergunta P7.

### 5.4 Quando o agente para, e como volta (D10, D11)

- **Gente respondeu → pausa** (`'gente'`), por **gatilho AFTER INSERT em `messages`**, com o predicado da 972: mensagem com `sender_id` **ou** `from_device`, sem `ia_agente_id`, fora de grupo. No banco, e não em código, pela lição da 972: são vários escritores (núcleo de envio, celular pareado, mensagem histórica recuperada). A carga de histórico da 1033 grava `gravada_em` nulo e fica de fora do gatilho por isso. O eco do próprio envio do agente não chega a ser gravado (sai no `jaGravada` da ingestão). A agendada sai com o `sender_id` de quem a criou: conta como gente. **O envio pela API v1 sai sem `sender_id`, de propósito** (`src/app/api/v1/messages/route.ts:188-193`), então **não pausa** — é a pergunta P6.
...
- ⚠️⚠️ **A transferência REACENDE a espera:** a última mensagem da conversa é do agente ("vou chamar um advogado"), que acabou de limpar o alerta. Sem reacender, o cliente transferido some da fila de atraso justamente quando passa a depender de gente. Uma função SQL (`cb_reacender_espera`), que reusa a fórmula do recálculo, grava a primeira fala do cliente depois da última resposta de gente, e é chamada depois do envio da despedida. Com isso ele volta ao "sem responsável" do Meu dia.
...
| Base de conhecimento | Os documentos marcados para este agente (pergunta P3) | `ai_knowledge_*` |
...
| `executar_automacao` | Só as automações da lista, com as guardas da rota manual. **Ao salvar a lista, recusa automação com passo fora da D5** (status ganho/perdido, etapa de resultado, `send_to_number`, `send_webhook`) | F4b | `runAutomationById`, `rotuloDoDisparo = 'ia:<agente>'` |
...
- **Trava:** o índice único parcial (um `aguardando` por conversa) e a reivindicação por `UPDATE … RETURNING` que exige não haver outro `rodando` na conversa, com cerca de posse (`rodando_desde`) em toda escrita e recolhedor de `rodando` velho — o padrão do Radar e do Calendly.
...
- **Antes de gerar e antes de enviar, confere de novo:** a pausa (o advogado pode ter respondido pelo celular), se o agente continua ativo e se chegou mensagem mais nova do cliente. Pausou → descarta (`pausado_no_meio`). Mensagem nova → descarta; o turno dela já está aguardando.
- **Áudio:** abre turno. A transcrição roda na hora, pela função idempotente da 943 (o "(futuro) auto-reply" já previsto em `.claude/rules/ia.md`), com a chave do Gemini da conta; se falhar, transfere para gente.
- **"Digitando…"** (só conexão Meta): chamado depois da espera de rajada, quando o turno vai gerar — não mais antes. Não há como cancelá-lo: numa transferência sem mensagem ele some sozinho em até 25 s.
...
- Por agente, na aba Uso: tokens por dia (produção e teste), turnos por desfecho (respondeu, transferiu, passou, falhou), iterações médias e as ações executadas. Custo em dinheiro: pergunta P4.
...
- **`/agents` vira a LISTA**: um cartão por agente com nome, descrição, modelo, conexões, se é agente de entrada de alguma, ligado/desligado e o uso de 7 dias. "Novo agente" em branco ou a partir de três modelos de partida (Triagem, Cobrança, Financeiro), com instruções iniciais no dicionário, sem o nome do escritório (o `produto-gate`).
- **`/agents/[id]` é o detalhe**, com as sub-abas de `src/components/settings/sub-abas.tsx`: **Configuração** (nome, descrição, instruções, provedor e modelo — só provedores com chave —, conexões, horário, teto, transferência, para quem pode passar), **Acesso** (F3), **Ferramentas** (F4), **Playground**, **Uso** e **Turnos** (o registro de cada resposta, com as ferramentas chamadas — é o que responde "por que a IA fez isso?").
...
- **Integrações:** as chaves por provedor e, em cada uma, os agentes e os módulos que a usam. O `FormularioDaChave` é reescrito na F1a: o eco dos campos do agente perde o sentido quando a chave sai de `ai_configs`.
...
- **Migration:** `cb_ia_chaves` com a cópia das chaves (5.2).
- **Servidor:** Radar, transcrição e embeddings leem a chave nova; Integrações grava e mostra por provedor; o "Remover" antigo some (apagar chave pede confirmação e diz o que para).
- **Testes:** pino de que Radar e transcrição resolvem a chave pelo provedor e não pelo canal; chave nunca na resposta de rota.
- **Pronto quando:** depois do deploy, uma análise do Radar e uma transcrição reais saem com a chave nova (conferido no `ai_usage_log`).
...
- **Migration:** `cb_ia_agentes` (com o gatilho de limpeza das conexões), colunas de agente em `ai_usage_log` e o `CHECK` de `mode`.
- **Servidor:** módulo `src/lib/ia-agentes/` (repositório, validação); rotas `/api/cb/ia/agentes*` (admin); Playground por agente registrando `agente_teste`; uso agregado no banco.
- **Tela:** lista e detalhe (Configuração, Playground, Uso).
- **Pronto quando:** na preview, criar os três agentes de exemplo, conversar com cada um no Playground com modelos diferentes, ver o uso de teste separado por agente, e os totais da aba Uso baterem com um `count`/`sum` direto no banco.

### F2 — Quem responde, e a resposta em texto

**Porta de entrada:** as perguntas P1, P2, P6 e P7 respondidas.

**Objetivo:** os agentes atendem de verdade, só com texto.

- **Migration:** `cb_channels.ia_agente_entrada_id`, `conversations.ia_agente_id`/`ia_pausada_por` (com o gatilho do encerramento), `messages.ia_agente_id`, `cb_ia_turnos`, o gatilho da pausa por gente, o ramo do agente nas três funções de "respondido", e `cb_reacender_espera`.
- **Servidor:** `quemResponde` (puro, com teste); a fila de turnos (rajada, trava, prazo, conferências antes de gerar e de enviar, disparo imediato + laço rápido); o portão de automação por "rodou"; o passo "Atribuir agente de IA"; o "Retomar IA" sem soltar a atribuição; a transferência com anotação e reacendendo a espera; a régua do Radar separando pendência de tempo de resposta; contexto só da conexão, com áudio transcrito e mídia descrita; `ia_agente_id` e `channel_id` no INSERT do `engineSendText`.
...
- **Piloto:** uma conexão só, com **dois** agentes (a triagem como entrada e mais um, atribuído por automação), por alguns dias, olhando os turnos. Qual conexão: pergunta P5.
...
- **Migration:** o vínculo documento ↔ agente (pergunta P3).
...
- **Pronto quando:** no Playground, o agente de cobrança vê as parcelas do contato escolhido e o de triagem não.
...
- **A transição (sem pausas herdadas):** nenhuma das 239 conversas está pausada, porque a pausa por gente só nasce depois da F2. A triagem ligada numa conexão falaria no meio de atendimentos que o advogado conduz pelo celular — a menos que a regra da P1 a restrinja.
...
## 8. Perguntas que sobram

Em cada uma, a recomendação. As quatro primeiras são a porta da F2.

- **P1. A quem o agente de ENTRADA atende?** Hoje, das 239 conversas abertas, 10 (cerca de 4%) têm responsável, e encerrar solta o responsável; mas o escritório fala com os clientes pelo celular pareado, que não atribui ninguém. (a) Só quem **nunca recebeu resposta de gente** (o lead novo); ou (b) também quem não fala com a equipe há N dias. *Recomendo (a): é o que a triagem é, resolve a transição (seção 7) e não deixa a IA falar com cliente que o advogado já atende. O cliente antigo só recebe agente quando uma automação o atribui.*
- **P2. O passo "Atribuir agente" retoma a IA numa conversa pausada por gente?** *Recomendo: retoma, exceto se alguém da equipe respondeu nas últimas 24 h; aí o agente fica atribuído mas pausado, e a faixa do fio mostra isso. O responsável humano nunca é tocado.*
- **P6. Mensagem enviada pela API v1 (o n8n, o Make) pausa a IA?** *Recomendo não: é máquina, como automação e disparo.*
- **P7. A régua de cobrança do Asaas pode atribuir o agente financeiro?** *Recomendo sim, com a regra da P2. Cuidado escrito: a régua roda a cada marco, então o teto do agente zera a cada cobrança.*
- **P3. Base de conhecimento:** cada documento marcado para os agentes que podem usá-lo? *Recomendo sim. Lembrete: a base exige a chave da OpenAI (o modelo de embeddings é fixo).*
- **P4. Custo:** tokens mais custo estimado em **dólar**, por uma tabela de preço no código ("sem preço" quando o modelo não está nela)? E um teto de gasto que pausa o agente fica para depois? *Recomendo as duas coisas assim.*
- **P5. Piloto da F2:** qual conexão, e quais dois agentes? *Pode responder quando a F2 estiver pronta.*

---

## 9. Diário

_(vazio — registrar cada fase ao concluir: data, PR, arquivos, resultado medido.)_
