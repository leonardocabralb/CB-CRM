...
> - **Fluxo por fase:** branch a partir de `main` → PR para o CB-CRM → revisão em duas lentes → merge. Um PR por fase. Migration aplicada ANTES do merge, depois do replay verde do CI, com número acima do maior no `main` (hoje 1041; a F1a usa a 1047).

## Estado

> ⚠️ **Números das migrations (26/09):** eram 1042/1043/1044; viraram **1047, 1048 e 1049** porque a `1044_cb_ligacoes` entrou no main e a 1046 foi aplicada por outras frentes — número abaixo do maior que está no main é recusado pelo `supabase db push` de quem instala.
...
| F1a | Chaves por provedor: Radar, transcrição, embeddings e Integrações leem a chave nova. Conferido em produção | migration aplicada (26/09); e2e no preview ok; merge a seguir | 1047 | #294 |
| F1b | Agentes (lista e detalhe), Playground e Uso por agente. **Nenhum agente responde ainda** | Codex limpo; espera o F1a | 1048 | #295 |
...
- **`cb_ia_turnos`** (F2): `account_id`, `conversation_id`, `mensagem_gatilho_id`, `canal_id`, `ia_agente_id` (o agente que respondeu; conversa, conexão e agente com **FK COMPOSTA pela conta**, `(x_id, account_id)`, SET NULL por coluna — um `uuid[]` de "agentes que agiram" não aceita FK por elemento e ficaria sem integridade nem recorte de conta; Codex, #292; a passagem da F4a, com teto de 1 por turno, ganha uma segunda coluna, `ia_agente_passado_id`, com a mesma FK), `modo` (`producao`/`teste`), `status` (`aguardando`, `rodando`, `respondeu`, `transferiu`, `sem_resposta`, `fora_do_horario`, `pausado_no_meio`, `descartado` (E10), `falhou`, `incerto`), `mensagem_enviada_id` (o id do provedor da resposta, gravado antes do INSERT — é o que a ingestão do eco consulta), `contexto` (jsonb, a partir da F2b: o RETRATO do que o modelo viu naquele turno — as instruções e regras do agente como estavam, os ids das mensagens da conversa que entraram, os blocos de acesso renderizados e os trechos da base recuperados, com o id do documento. Sem isso a sub-aba Turnos não explica "por que a IA fez isso?" depois que a ficha, o card ou o documento mudarem — Codex, #292. É dado de cliente: a tabela continua fechada ao navegador, o retrato tem teto de tamanho, e a tabela ganha poda de 90 dias), `executar_apos`, `rodando_desde` (a posse), `iteracoes`, `acoes` (jsonb: ferramenta, argumentos, resultado resumido, ok/erro), tokens, `erro`. **Dois índices únicos parciais**: `(conversation_id, canal_id) WHERE status = 'aguardando'` (a rajada: um turno esperando POR CONEXÃO, com `NULLS NOT DISTINCT`; é o alvo do `ON CONFLICT` da RPC) e `(conversation_id) WHERE status = 'rodando'` (um executando por conversa). Fechada ao navegador; a tela lê por rota de admin.
...
- **Link que não veio do servidor não sai.** Toda URL na resposta tem de ter aparecido, naquele turno, num resultado de ferramenta, no acesso do agente, **nas instruções ou regras do agente, ou num trecho da base de conhecimento recuperado para o turno** (Codex, #292: o link aprovado que o administrador escreve nas instruções ou num documento é legítimo); senão a resposta é retida e o turno falha visível. A régua compara com o texto EXATO do que entrou no pedido. Modelo inventa link de boleto.
...
| `executar_automacao` | Só as automações da lista, com as guardas da rota manual. **Ao salvar a lista, recusa automação com passo fora da D5** (status ganho/perdido, etapa de resultado, `send_to_number`, `send_webhook`), **e confere DE NOVO na hora de executar** (Codex, #292): a automação aprovada pode ser editada depois, e `runAutomationById` roda a definição de agora. A conferência percorre também as automações que ela aciona (`run_automation`); passo proibido encontrado = a ferramenta recusa e o turno registra por quê. ⚠️ **E na RETOMADA de um "Aguardar"** (Codex, #292): `resumePendingExecution` relê a automação e segue da posição guardada, então um passo proibido acrescentado DEPOIS do disparo escaparia. A execução que a IA iniciou leva a origem no contexto (`origem_ia`: o agente e o turno), e a retomada confere a D5 de novo antes de cada passo — **e confere a CONVERSA também** (Codex, #292): a conferência de antes de cada ferramenta (5.7) roda dentro do laço do modelo, que já terminou quando o "Aguardar" acorda. Antes de cada passo de uma execução com `origem_ia`, na retomada e no fluxo normal, a conversa tem de estar sem pausa e com o MESMO agente ativo **na MESMA atribuição** — `origem_ia` guarda também a GERAÇÃO da atribuição do disparo (`conversations.ia_atribuicao`, um contador que entra com esta ferramenta, na F4b), e ela é comparada (Codex, #292): encerrar zera a atribuição (E11), e o mesmo agente atribuído de novo a um atendimento novo não pode retomar a execução do antigo. ⚠️ Não é o `ia_agente_desde`, que a E12 MANTÉM quando o mesmo agente é reatribuído: a geração avança em TODA atribuição que muda algo — retomar uma pausa ou zerar o teto de respostas — e só fica igual na reatribuição que não muda nada; senão a automação da IA parada num "Aguardar", depois de a pausa de gente ser retomada por outra automação 24 h depois, voltaria a rodar no atendimento novo; pausada (gente respondeu, botão, transferência), com outro agente ou sem agente, a execução para ali, com a marca de interrupção da 1005 (motivo novo `ia_pausada`, no CHECK) e a anotação `interrupcao.ts` — nunca um passo que fala com o cliente depois de o advogado ter respondido (D10) | F4b | `runAutomationById`, `rotuloDoDisparo = 'ia:<agente>'` |
...
- **Idempotência:** o turno nasce do INSERT da mensagem, que já é único; a reentrega do provedor não cria turno. ⚠️ **Limite aceito** (Codex, #292): o turno é enfileirado DEPOIS do robô e das automações, na mesma requisição que gravou a mensagem; um processo que morre entre as duas (deploy com o `after()` passando dos 10 s do `stop_grace_period`, queda) deixa a mensagem sem turno, e a reentrega do provedor não conserta (a mensagem já existe e a ingestão sai no `jaGravada`/23505). Criar o turno JUNTO com a mensagem (gatilho ou outbox) não é possível: quem decide se há turno é o que o robô e as automações fizeram com ELA (E4), e isso só se sabe depois de eles rodarem; reconciliar na reentrega decidiria sem esse dado. O lado da falha é o seguro — nenhuma resposta da IA, nunca duas —, e a espera acesa pela mensagem (`aguardando_desde`, 972) põe a conversa no alerta de atraso para a equipe em 10 min. É a mesma janela que a religação da 1010 já aceita.
...
- **Antes de gerar e antes de enviar, confere de novo:** o AGENTE, relido (ligado, não arquivado e com a conexão do turno nas `conexoes`: desligar o agente ou tirar a conexão dele não zera `conversations.ia_agente_id`, e a decisão da entrada já é velha depois da espera e da geração — Codex, #292), se a mensagem que abriu o turno continua lá (o cliente pode tê-la APAGADO na espera ou na geração, e o contexto já não a mostra: descarta — Codex, #292), a pausa (o advogado pode ter respondido pelo celular), se o agente continua ativo, se chegou mensagem mais nova do cliente e **se chegou resposta de GENTE gravada depois da mensagem do cliente** — pela ordem de GRAVAÇÃO (`gravada_em`), com pausa `'gente'` se o gatilho não a gravou. É o que fecha a corrida da ENTRADA: a resposta do advogado que chega entre a leitura da D16 e a atribuição é gravada sem agente ativo (o gatilho não tem o que pausar), e a mensagem do celular traz o relógio do aparelho, que pode ser anterior ao `ia_agente_desde` (Codex, #292). A resposta de gente sempre vence (D10). Pausou → descarta (`pausado_no_meio`). Mensagem nova → descarta; o turno dela já está aguardando. ⚠️ **A última palavra é do BANCO, atômica com o envio** (Codex, #292): conferir em JS e só depois enviar deixa uma janela em que a resposta do advogado é gravada no meio e a IA envia assim mesmo. Por isso a vaga do teto é consumida por uma RPC (`cb_ia_reservar_envio`) que, NA MESMA escrita em `conversations`, exige a conversa aberta, sem pausa e com o mesmo agente; a pausa por gente (o gatilho) escreve a mesma linha, e as duas se serializam pela trava dela. Resposta de gente gravada antes da reserva a recusa; gravada depois é a simultaneidade que nenhum banco evita (a mensagem da IA já estava autorizada e em voo). **Com ferramentas (F4), cada efeito reserva do mesmo jeito** antes de agir. ⚠️ **Com ferramentas (F4), a mesma conferência roda antes de CADA ferramenta com efeito** (Codex, #292): descartar o texto no fim não desfaz uma etapa movida, uma tarefa criada ou uma automação executada. Mudou algo → o laço para ali, sem executar a ferramenta.
...
- **Migration (1047):** `cb_ia_chaves` com a cópia das chaves (5.2) e `ai_configs.api_key` sem NOT NULL (a linha padrão passa a existir sem chave). A coluna antiga FICA com o valor de hoje: se o deploy voltar atrás, o app anterior acha a chave onde sempre achou.
...
- **E4. Portão das automações:** o motor devolve `falou` no `ResultadoDoDisparo`, marcado no MOMENTO do passo: envio que deu certo, envio reenfileirado pela retentativa, e — por conservadorismo — `run_flow`, `run_automation` e "Aguardar" (vão ou podem falar depois). Contam todos os gatilhos da mensagem: nova mensagem, palavra-chave, primeira mensagem, contato novo e resposta de botão. **"Mover card" e "Criar negócio" contam como fala quando alguma automação LIGADA de `deal_stage_changed` dispara para a etapa de destino** (a régua de `triggerMatches`, lista vazia = toda etapa; sem olhar os passos dela, como no `run_automation`), e a leitura que falha conta como fala (Codex, #292): esses passos só gravam o evento do funil, e a automação da etapa roda depois, no dreno — o agente responderia antes dela. A automação de `tag_added` acionada por "Adicionar etiqueta" **propaga a SUA fala** para a de cima (Codex, #292): se ela mandou mensagem ao contato, a automação que etiquetou conta como tendo falado; se ela só etiqueta (ou não há `tag_added` escutando), nada muda — contar a mera etiqueta calaria o agente em toda automação que só etiqueta.
...
- **E9. Áudio:** o turno só chama a transcrição com o arquivo já baixado; sem ele, reagenda por até 2 min contados de `gravada_em` (o relógio do banco), e depois transfere. `transcrevendo` reagenda. Imagem, documento e vídeo entram como descrição (`[documento: extrato.pdf]`); figurinha, localização, texto sem conteúdo (cartão de contato, enquete e resposta de botão chegam assim pela Evolution) e toque em botão não abrem turno. As duas ingestões gravam a figurinha como `image`: a ingestão passa à entrada o fato "é figurinha" e "tem texto", e a régua é uma só para a entrada e para a E10.
- **E10. Mensagem mais nova do cliente NA MESMA CONEXÃO** (por `gravada_em`) **que ABRE turno** (a régua da E9: figurinha, localização, texto sem conteúdo e toque em botão não contam, nem mensagem apagada) descarta o turno em curso (`descartado`, status novo): o turno dela já está na fila. Mensagem que não abre turno não descarta nada (Codex, #292): o cliente que manda um texto e uma figurinha ficaria sem resposta. A régua é UMA, compartilhada pela entrada e pelo turno. ⚠️ **Mas alguém já ter respondido descarta** (Codex, #292): o cliente manda texto (turno na fila) e toca num botão na mesma rajada; o toque não abre turno, e a automação de "resposta de botão" responde. Sem mais nada, o turno do texto responderia de novo. Então (a) a mensagem que o robô consumiu ou à qual uma automação respondeu CANCELA o turno pendente da mesma conversa e conexão (`descartado`), e (b) o turno em curso, nas duas conferências, descarta quando há saída do robô ou de automação (`sender_type = 'bot'` sem `ia_agente_id`) **pela MESMA conexão do turno** (Codex, #292: a resposta de uma automação pela conexão B não substitui nada na A, e o contexto é por conexão, D4) gravada depois da mensagem dele. Custo aceito: um lembrete ou uma cobrança da régua que saia pela mesma conexão naquele instante também cala o agente naquela mensagem.
...
- **E12. "Atribuir agente" decide a D17 no BANCO** (`cb_atribuir_agente_de_ia`, com a conversa travada): ler "houve resposta de gente em 24 h?" em JS e depois gravar deixaria uma resposta do advogado no meio sem pausar. Pausa sem motivo (anterior à 1049) conta como `'botao'` (não retoma). Reatribuir o MESMO agente mantém o `ia_agente_desde` (a janela da pausa por gente); a GERAÇÃO da atribuição (`ia_atribuicao`, F4b) avança sempre que a reatribuição retoma uma pausa ou zera o teto — é ela que invalida a execução antiga iniciada pela IA (Codex, #292).
