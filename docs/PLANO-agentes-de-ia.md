...
| F2 | Quem responde: agente de entrada da conexão, agente ativo da conversa, passo "Atribuir agente", pausa por gente, fila de turnos, resposta em texto. Piloto na **Bancário - Comercial** (D22) | a fazer | sim | — |
...
- **Cotação do dólar** (F1b, D21): `ai_configs.cotacao_dolar numeric` na linha padrão, gravada por rota PRÓPRIA que só mexe nessa coluna (o `POST /api/ai/config` reescreve a linha inteira). Nula = custo não calculado.
...
- **`cb_ia_agente_documentos`** (F3, D20): `(account_id, ia_agente_id, documento_id)`, FKs compostas pela conta, CASCADE dos dois lados (apagar o documento tira o vínculo; arquivar o agente não apaga nada, e o vínculo fica para quando ele voltar). Um documento pode servir a vários agentes. A busca na base (`match_ai_knowledge_semantic`/`_fts`) ganha o recorte pelo agente: só os trechos dos documentos marcados para ele. **Nada marcado = nenhuma base** (o mesmo "fechado por padrão" do acesso).
...
5. Senão, a conexão tem **agente de entrada** e a conversa **nunca recebeu resposta de gente** (D16: nenhuma mensagem com `sender_id` ou `from_device`, em qualquer conexão da conversa, apagada inclusive — a agendada conta, porque alguém a escreveu; a da API v1, o robô e o disparo não) → **ele**, e ele vira o agente ativo. O histórico importado da Kommo (1033) grava as mensagens da equipe como `from_device`, então quem tem histórico não cai na triagem.
6. Senão → **sem IA**.
...
**Automação aciona o agente (D9):** um passo NOVO, **"Atribuir agente de IA: X"** — não a mudança do `set_ai` de hoje, que ao ligar apaga o responsável humano. O passo grava o agente ativo e zera o teto, **não toca `assigned_agent_id`**, e trata a pausa pela D17: retoma, salvo resposta de gente nas últimas 24 h — aí o agente fica atribuído e pausado. O `set_ai` legado continua como está (ligar/desligar), e o "Retomar IA" do cabeçalho deixa de soltar a atribuição, porque ela deixou de ser portão. **A régua do Asaas pode usar o passo (D19)**: ele entra na lista do que o `validate.ts` aceita nos dois gatilhos da régua e fica FORA de `PASSOS_QUE_FALAM_COM_O_CONTATO` (atribuir não é cobrar: a trava do marco continua medindo pela mensagem). ⚠️ A régua roda a cada marco, então o teto de respostas do agente zera a cada cobrança — aceito.
...
- **Migration:** `cb_ia_agentes` (com instruções e regras, e o gatilho de limpeza das conexões), colunas de agente em `ai_usage_log`, o `CHECK` de `mode` e `ai_configs.cotacao_dolar`.
