...
import { ATRASO_DE_RESPOSTA_MS, atrasoDeResposta } from "./atraso";
...
describe("atrasoDeResposta — a régua dos 10 minutos", () => {
  ...
  it("abaixo de 10 minutos ainda não alerta; em 10 alerta", () => {
    ...
    expect(atrasoDeResposta(aberta(min(10)), AGORA)).toEqual({ n: 10, unidade: "min" });
  });

  it("escolhe a unidade mais legível", () => {
    expect(atrasoDeResposta(aberta(min(59)), AGORA)).toEqual({ n: 59, unidade: "min" });
    expect(atrasoDeResposta(aberta(min(60)), AGORA)).toEqual({ n: 1, unidade: "h" });
    expect(atrasoDeResposta(aberta(min(23 * 60 + 59)), AGORA)).toEqual({ n: 23, unidade: "h" });
    expect(atrasoDeResposta(aberta(min(48 * 60)), AGORA)).toEqual({ n: 2, unidade: "d" });
  });
