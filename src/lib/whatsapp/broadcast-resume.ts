...
export async function planBroadcastResume(
  ...
  const { data: broadcast, error: bcError } = await db
    .from('broadcasts')
    .select('id, template_name, template_language, channel_id')
    .eq('id', broadcastId)
    .eq('account_id', accountId)
    .maybeSingle();
  ...
  // MULTI-CANAL: a retomada tem que sair pelo MESMO numero que iniciou a
  // campanha, nao pelo padrao da conta. `broadcasts.channel_id` (903) guarda
  // esse numero; sem isto, retomar uma campanha do numero Comercial a
  // terminaria pelo numero do socio — do ponto de vista do cliente, uma
  // mensagem que comecou em um contato e continuou em outro.
  //
  // `resolveMetaChannel` cai para o canal padrao quando `channel_id` e NULL
  // (campanha anterior a 903), que e exatamente o comportamento do upstream.
  const canal = await resolveMetaChannel(
    db,
    accountId,
    (broadcast as { channel_id?: string | null }).channel_id ?? null
  );
  if (!canal) {
    throw new BroadcastError(
      'whatsapp_not_configured',
      'Resuming a broadcast needs an official Meta (Cloud API) number — connect one in Settings > Connections.',
      400
    );
  }
  ...
  const plan: BroadcastPlan = {
    ...
    accessToken: decrypt(canal.accessToken),
