...
export async function resolveTemplateRow(
  ...
  const todas = ((Array.isArray(data) ? data : []) as {
    language?: string;
  ...
  // Recorte por canal (migration 903). O catálogo da Meta é POR WABA, então o
  // modelo tem que ser o DAQUELE número: sem isto, numa conta com dois WABAs o
  // atendente via o preview de um e o cliente recebia o texto do outro — ou
  // nada, com a Meta devolvendo "template does not exist".
  //
  // Modelo com `channel_id` NULL é global (anterior à 903) e continua valendo
  // como reserva; o do canal tem prioridade. Quem não passa canal enxerga tudo,
  // que é o comportamento do upstream e o de antes da 903.
  const doCanal = channelId
    ? todas.filter((t) => t.channel_id === channelId)
    : [];
  const globais = channelId ? todas.filter((t) => t.channel_id == null) : todas;
  const rows = doCanal.length > 0 ? doCanal : globais;
