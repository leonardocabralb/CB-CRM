...
describe('celularDigitado — o celular que o membro digita para si', () => {
  ...
  it('número de fora do Brasil, escrito com o código do país, passa como veio', () => {
    expect(celularDigitado('+351 912 345 678')).toEqual({
      ok: true,
      digitos: '351912345678',
    });
    expect(celularDigitado('+1 404 555 1234')).toEqual({
      ok: true,
      digitos: '14045551234',
    });
  });

  it('sem o + (ou 00), o número tem de ser brasileiro — um dígito a mais não vira "estrangeiro"', () => {
    // Celular com um dígito a mais: 12 dígitos sem 55, que a régua das telas
    // devolveria como veio.
    expect(celularDigitado('11 91234-56789')).toEqual({
      ok: false,
      motivo: 'invalido',
    });
    // Fixo com um dígito a mais: 11 dígitos sem o 9 na 3ª posição.
    expect(celularDigitado('(11) 34567-8901')).toEqual({
      ok: false,
      motivo: 'invalido',
    });
    // Número de fora escrito sem o +.
    expect(celularDigitado('351912345678')).toEqual({
      ok: false,
      motivo: 'invalido',
    });
    // Com o +, o mesmo número de fora passa.
    expect(celularDigitado('+351912345678')).toEqual({
      ok: true,
      digitos: '351912345678',
    });
  ...
  it('sem o DDD é "curto" — nunca vira número de outro país', () => {
    expect(celularDigitado('91234-5678')).toEqual({
      ok: false,
      motivo: 'curto',
    });
  });
  ...
  it('fixo brasileiro não é celular', () => {
    expect(celularDigitado('(11) 3456-7890')).toEqual({
      ok: false,
      motivo: 'nao_e_celular',
    });
    expect(celularDigitado('+55 11 3456-7890')).toEqual({
      ok: false,
      motivo: 'nao_e_celular',
    });
  });
  ...
  it('o celular antigo, sem o 9, é recusado — tem a mesma forma do fixo', () => {
    expect(celularDigitado('(11) 8765-4321')).toEqual({
      ok: false,
      motivo: 'nao_e_celular',
    });
    expect(celularDigitado('551187654321')).toEqual({
      ok: false,
      motivo: 'nao_e_celular',
    });
  });
  ...
  it('letra, 0 de tronco e 0800 são "invalido"', () => {
    expect(celularDigitado('11 9 1234 ramal 5')).toEqual({
      ok: false,
      motivo: 'invalido',
    });
    expect(celularDigitado('011 91234-5678')).toEqual({
      ok: false,
      motivo: 'invalido',
    });
    expect(celularDigitado('0800 123 4567')).toEqual({
      ok: false,
      motivo: 'invalido',
    });
  });
