...
| `src/lib/whatsapp/template-webhook.ts` e a chamada em `src/app/api/whatsapp/webhook/route.ts` (Fase 6b do merge do upstream, 24/09/2026) | o STUB do modelo criado direto no painel da Meta (#534). O original resolve a conta por `whatsapp_config.waba_id` — o espelho de UM número, que nesta produção nunca casa — e grava o stub sem canal, com o `user_id` da config. O NOSSO: a conexão sai de `cb_channels.waba_id` (`kind = 'meta'`, exatamente UMA — dois números na mesma WABA viram só log, como o original faz com duas configs), o stub nasce com o `channel_id` (senão seria um modelo "global" ao lado do que a sincronização cria para o canal) e com o DONO da conta em `user_id` (`message_templates.user_id` CASCADEia de `auth.users`), e NÃO nasce se já existe linha de mesmo nome e idioma no canal, de qualquer autor — o índice único da 903 leva o `user_id`, e o stub do dono passaria por ele ao lado do modelo de outro admin. A rota passa `wabaId: entry.id` (sem ele o stub é inerte, o estado do #259). Um merge que traga o arquivo cru devolve o `whatsapp_config` sem conflito. Pino em `template-webhook.test.ts` (lê a rota e o módulo) |
| `src/lib/whatsapp/template-header-handle.ts` (Fase 6a, 24/09/2026) | a amostra do cabeçalho de mídia é lida por `lerComTeto` (`src/lib/http/ler-com-teto.ts`), com o `content-length` conferido antes: o original lia o corpo INTEIRO (`arrayBuffer`) antes do teto de 100 MB do documento, com a URL colada por qualquer membro. Um merge que traga a leitura crua devolve o processo derrubável por uma URL. Pino em `template-header-handle.test.ts` (um corpo sem fim para no teto) |
...
- ⚠️⚠️ **A URL do anexo NÃO é confiável — ela vem do CORPO que a própria
  ...
  nunca por `arrayBuffer()`. A guarda (`src/lib/webhooks/ssrf.ts`,
