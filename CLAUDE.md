...
- `docs/` — a documentação ENTREGUE a quem instala o sistema: `README.md`
  ...
  `docker.md`, `public-api.md`, `mcp.md`, `webhooks.md`, `multi-waba.md`
  (vários números oficiais; entrou na Fase 3c do plano do upstream) e
  `conexao-meta.md` (por que a conexão oficial falhou, frase a frase; Fase 7).
  ⚠️ Até 2026-09-08 esta linha dizia
...
- ⚠️⚠️ **Só o transporte EVOLUTION retenta hoje.** A régua
  ...
  `httpStatus`, `code` e `subcode`), mas ligar a Meta NÃO é trocar a classe:
  um 4xx da Meta costuma ser DETERMINÍSTICO (janela de 24 h fechada, número
  que não recebe, parâmetro recusado), e repeti-lo só adia o aviso — seria
  uma régua por `code`. A Fase 7 do plano do merge do upstream (24/09/2026)
  deixou isso FORA do plano, como decisão do operador; até lá a régua falha
  FECHADA para a Meta e não repete.
...
- ⚠️⚠️ **O telefone passa pela régua das telas (`telefoneDigitado`), não pela
  dos sistemas** (Fase 3-III, pedido do operador em 23/09/2026: o formulário do
  Typebot não é nosso para mudar). Com `digitosDoTelefone`, "98874-5316" (sem
  DDD) virava a ficha +98 e um `…@lid` colado virava telefone. O bloco Phone do
...
- ⚠️⚠️ **A URL do anexo NÃO é confiável — ela vem do CORPO que a própria
  ...
  `template-header-handle.ts`, e aqui), e o CORPO que vai para a memória
  passa por `lerComTeto` (`src/lib/http/ler-com-teto.ts`, desde a Fase 6a),
  nunca por `arrayBuffer()` — exceção conhecida, anterior: a foto de perfil
  (`foto-do-contato.ts`) lê a URL da Evolution inteira antes de conferir
  `FOTO_MAX_BYTES`. A guarda (`src/lib/webhooks/ssrf.ts`,
