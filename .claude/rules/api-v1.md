...
- ⚠️⚠️ **VAZIO NÃO APAGA na v1 (decisão do operador, 26/09/2026).** Em `PATCH /contacts/{id}/custom-fields`, `""`, só-espaços e `null` em `values` NÃO mexem no campo; apagar é a lista `clear` (chave desconhecida ou a mesma chave com valor e em `clear` = 400), e corpo só com vazios responde 200 sem escrever. Em `PATCH /contacts/{id}`, `name`/`email`/`company` são APARADOS e texto vazio não mexe (`null` segue limpando); no `POST /contacts`, aparados, e nome vazio cai no telefone. Motivo medido: o Typebot da iMotion manda TODAS as variáveis a cada passo, o "Create JSON" do Make troca resposta não dada por `null`, e o lead que recomeçava o formulário perdia as respostas (35 DELETEs num dia); e o nome digitado com espaço no fim ("ALINI ") passava por cima do fixado pelo Calendly. Voltar a apagar com vazio reabre os dois — a UI (`salvarValoresDoContato`) continua com `""` = apagar, de propósito: lá o vazio é gesto de gente.
